...
func (r Rect) Intersection(other Rect) Rect {
	...
	return NewRect(x, y, right-x, bottom-y)
}
...
func (r Rect) Offset(dx, dy int) Rect {
	return NewRect(r.X+dx, r.Y+dy, r.Width, r.Height)
}
...
func (r Rect) SplitHorizontal(offset int) (Rect, Rect) {
	...
	top := NewRect(r.X, r.Y, r.Width, offset)
	bottom := NewRect(r.X, r.Y+offset, r.Width, r.Height-offset)
	return top, bottom
}
...
func (r Rect) SplitVertical(offset int) (Rect, Rect) {
	...
	left := NewRect(r.X, r.Y, offset, r.Height)
	right := NewRect(r.X+offset, r.Y, r.Width-offset, r.Height)
	return left, right
}
